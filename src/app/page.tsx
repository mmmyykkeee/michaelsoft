    ...
    <main className="flex-1 flex flex-col items-center justify-center p-8 text-center" id="app">
      ...
      <header className="fixed top-0 left-0 w-full px-8 md:px-20 py-8 md:py-12 flex justify-between items-center z-10">
        <div className="flex items-center gap-3 md:gap-4 group cursor-default">
          ...
          <div className="font-headline font-bold tracking-widest_extra text-[0.6rem] md:text-[0.7rem] transition-all duration-300 group-hover:tracking-[0.2em] group-hover:text-white">
            MichaelSoft
          </div>
        </div>
        <nav className="hidden md:block">
          <ul className="flex list-none gap-8 items-center text-[0.65rem] tracking-widest_extra">
            <li>
              <a 
                href="#vision" 
                className="no-underline text-on-surface-variant font-headline font-medium text-[0.65rem] tracking-widest_extra transition-all duration-300 hover:text-on-surface relative peer"
            ...
            <li>
              <a 
                href="/projects" 
                className="flex items-center gap-3 no-underline text-on-surface-variant font-headline font-medium text-[0.65rem] tracking-widest_extra transition-all duration-300 hover:text-on-surface relative peer"
              >
                Projects Archive
        ...
        <div className="flex gap-8 items-center">
          ...
          <div className="relative" ref={desktopContactRef}>
            <button 
              ...
              className={`hover:text-on-surface transition-colors duration-300 no-underline tracking-v_widest cursor-pointer ${contactOpen ? 'text-on-surface' : ''}`}
            >
              CONTACT
            </button>
      ...
      <nav className="mobile-nav hidden max-[768px]:flex flex-col fixed bottom-0 left-0 w-full px-6 pt-5 pb-6 bg-[#0a0f13]/80 backdrop-blur-3xl border-t border-white/5 z-[100] gap-5">
        <div className="flex justify-around items-center w-full">
          <a href="#vision" className="flex flex-col items-center gap-2 text-white/40 text-[0.6rem] tracking-widest_extra font-headline font-semibold hover:text-on-surface transition-colors group">
          ...
          <a href="/projects" className="flex flex-col items-center gap-2 text-white/40 text-[0.6rem] tracking-widest_extra font-headline font-semibold hover:text-on-surface transition-colors group">
          ...
          <div className="relative flex flex-col items-center" ref={mobileContactRef}>
            ...
            <button 
              onClick={() => setContactOpen(!contactOpen)} 
              className={`flex flex-col items-center gap-2 text-[0.6rem] tracking-widest_extra font-headline font-semibold transition-colors group cursor-pointer ${contactOpen ? 'text-on-surface' : 'text-white/40 hover:text-on-surface'}`}
            >
        ...
        <div className="flex justify-center items-center w-full border-t border-white/5 pt-4">
          <span className="text-[0.45rem] tracking-[0.2em] text-white/30 font-headline">
